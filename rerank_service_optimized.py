...
def generate_request_id(instruction, query, doc):
    """生成基于内容的稳定request_id，用于KV缓存"""
    content = f"{instruction}:{query}:{doc}"
    return f"rerank_{hashlib.md5(content.encode()).hexdigest()[:16]}"

async def compute_logits_with_cache(engine, messages, sampling_params, true_token, false_token, instruction, query):
    """使用KV缓存的批处理版本"""
    try:
        scores = []
        ...
        for i, message in enumerate(messages):
            # 从message中提取document内容（这里需要根据实际token结构调整）
            # 简化处理：使用索引作为document标识
            doc_content = f"doc_{i}"  # 实际使用时需要从message中提取
            request_id = generate_request_id(instruction, query, doc_content)
...
async def compute_logits_sequential(engine, messages, sampling_params, true_token, false_token, instruction, query):
    """顺序处理版本 - 用于小批量，但使用稳定的request_id"""
    try:
        scores = []
        
        for i, message in enumerate(messages):
            # 生成稳定的request_id
            doc_content = f"doc_{i}"
            request_id = generate_request_id(instruction, query, doc_content)
...
async def initialize_model(config: ModelConfig = None):
    ...
    engine_args = AsyncEngineArgs(
        ...
        block_size=16,  # 设置block大小，影响KV缓存效率
        swap_space=4,  # 交换空间大小
...
# FastAPI应用
app = FastAPI(
    title="Rerank Service (KV Cache Optimized)", 
    description="启用KV缓存的单GPU文档重排序服务",
    lifespan=lifespan
)
...
async def root():
    """根路径"""
    return {"message": "Rerank Service is running (KV Cache Optimized)"}
...
async def rerank_documents(request: RerankRequest):
    ...
    try:
        ...
        if len(inputs) > 10:  # 大批量使用批处理
            scores = await compute_logits_with_cache(engine, inputs, sampling_params, true_token, false_token, request.instruction, request.query)
        else:  # 小批量使用顺序处理
            scores = await compute_logits_sequential(engine, inputs, sampling_params, true_token, false_token, request.instruction, request.query)
...
async def batch_rerank(batch_requests: List[RerankRequest]):
    ...
    try:
        ...
        # 批量计算scores - 使用KV缓存
        scores = await compute_logits_with_cache(engine, inputs, sampling_params, true_token, false_token, all_instructions[0], all_queries[0])
